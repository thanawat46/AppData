...
class _IncomeState extends State<Income_year> {
                ...
                Expanded(
                  child: FutureBuilder<List<CaneData>>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      // คำนวณข้อมูลสรุป (แยก Logic ออกมาให้โค้ดสะอาด)
                          ...
                          slivers: [
                            ...
                            SliverPersistentHeader(
                              ...
                              delegate: _TableHeaderDelegate(
                                minHeight: 60,
                                maxHeight: 60,
                                child: _buildTableHeader(totalTableWidth),
                              ),
                            ),

                            // 3. Body Data
                            SliverToBoxAdapter(
                              child: _buildTableBody(data, totalTableWidth),
                            ),
  ...
  // --- Helper Methods (Code Extraction) ---

  Map<String, dynamic> _calculateSummary(List<CaneData> data) {
      ...
      child: Column(
        ...
        children: [
          ...
          TextButton(
            ...
            child: const Text("ลองใหม่", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          )
        ],
      ),
    );
  ...
  Widget _buildDashboard(Map<String, dynamic> summary) {
    return Column(
      children: [
        _buildPremiumIncomeCard(summary['totalIncome']),
        const SizedBox(height: 15),
        Row(
          children: [
            Expanded(child: _buildSummaryCard(Icons.local_shipping, "เที่ยววิ่ง", integerFormatter.format(summary['count']), "เที่ยว", const Color(0xFFD32F2F))),
            const SizedBox(width: 10),
            Expanded(child: _buildSummaryCard(Icons.scale, "นน.รวม", numberFormatter.format(summary['totalTons']), "ตัน", Colors.orange[800]!)),
            const SizedBox(width: 10),
            Expanded(child: _buildSummaryCard(Icons.science, "CCS เฉลี่ย", (summary['averageCCS'] as double).toStringAsFixed(2), "%", Colors.teal[700]!)),
          ],
        ),
  ...
  Widget _buildTableBody(List<CaneData> data, double width) {
    return Container(
      color: Colors.white,
      child: SingleChildScrollView(
